...
import com.google.blockly.android.ui.fieldview.ListRequestCallback;
...
public class BlocklyController {
    ...
    private BlocklyController(Context context, BlockFactory blockModelFactory,
        ...
        if (mViewFactory != null) {
            ...
            mViewFactory.setListNameManager(mWorkspace.getListNameManager());
            mViewFactory.setListRequestCallback(new ListRequestCallback() {
                @Override
                public void onListRequest(int request, String list) {
                    if(request == ListRequestCallback.REQUEST_RENAME) {
                        requestRenameList(list, list);
                    } else if(request == ListRequestCallback.REQUEST_DELETE) {
    ...
    /*
    * @param listCallback The callback to notify when a variable is being deleted.
    */
    ...
    public boolean isListInUse(String list) {
        return mWorkspace.getListRefCount(list) > 0;
    }
    ...
    public List<Block> getBlocksWithList(String list) {
        return mWorkspace.getBlocksWithList(list, null);
    }
    ...
    public String addList(String list) {
        checkPendingEventsEmpty();
        String result = addListImpl(list, true);
        firePendingEvents();
        return result;
    }
    ...
    public String requestAddList(String list) {
        checkPendingEventsEmpty();
        String result = addListImpl(list, false);
        firePendingEvents();
        return result;
    }
    ...
    public boolean deleteList(String list) {
        checkPendingEventsEmpty();
        boolean result = deleteListImpl(list, true);
        firePendingEvents();
        return result;
    }
    ...
    public boolean requestDeleteList(String list) {
        checkPendingEventsEmpty();
        boolean result = deleteListImpl(list, false);
        firePendingEvents();
        return result;
    }
    ...
    public String renameList(String list, String newList) {
        checkPendingEventsEmpty();
        String result = renameListImpl(list, newList, true);
        firePendingEvents();
        return result;
    }
    ...
    public String requestRenameList(String list, String newList) {
        checkPendingEventsEmpty();
        String result = renameListImpl(list, newList, false);
        firePendingEvents();
        return result;
    }
    ...
    /**
     * Implements {@link #addList(String)}, without firing events.
     *
     * @param list The desired name of the variable to create.
     * @param forced True to skip the variable callback and add the given variable name immediately.
     *               False to request confirmation from the callback first.
     * @return The actual variable name that was created or null if creation was blocked.
     */
    private String addListImpl(String list, boolean forced) {
        Log.i("dd",list);
        if(!forced) {
            Log.i("dd","dd2");
            if(!mListCallback.onCreateList(list)) {
                Log.i("dd","dd3");
                return null;
            }
        }
        Log.i("dd","dd4");
        return mWorkspace.getListNameManager().generateUniqueName(list, true);
    }
    ...
    private boolean deleteListImpl(String list, boolean forced) {
        if(!forced && mListCallback != null) {
            if(!mListCallback.onDeleteList(list)) {
                return false;
            }
        }
        if(isListInUse(list)) {
            mTempBlocks.clear();
            List<Block> blocks = mWorkspace.getBlocksWithList(list, mTempBlocks);
            for(int i = 0; i< blocks.size(); i++) {
                removeBlockAndInputBlocksImpl(blocks.get(i));
            }
        }
        return mWorkspace.getListNameManager().remove(list);
    }
    ...
    private String renameListImpl(String list, String newList, boolean forced) {
        if(!forced && mListCallback != null) {
            if(!mListCallback.onRenameList(list, newList)) {
                return list;
            }
        }
        if(TextUtils.isEmpty(newList) || list == newList) {
            return list;
        }
        newList = addListImpl(newList, true);
        List<FieldList> listRefs = mWorkspace.getListRefs(list);
        if(listRefs != null) {
            int count = listRefs.size();

            for(int i = 0; i < count; i++) {
                FieldList field = listRefs.get(i);
    ...
    public static class Builder {
        ...
        public Builder setListCallback(ListCallback listCallback) {
            mListCallback = listCallback;
            return this;
        }
        ...
        public BlocklyController build() {
            ...
            controller.setListCallback(mListCallback);
    ...
    public abstract static class ListCallback {
         ...
         * any blocks referencing it will be deleted.
         *
         * @param list The variable being deleted.
         * @return True to allow the delete, false to prevent it.
         */
        public boolean onDeleteList(String list) { return true; }
         ...
         * set to true.
         *
         * @param listName The initial variable name or null if no starting name was specified.
         * @return True to create the named variable, false to handle it yourself.
         */
        public boolean onCreateList(String listName) { return true; }
         ...
         * variables.
         *
         * @param list The variable to rename.
         * @param newList The new name for the variable.
         * @return True to perform the rename, false to handle it yourself.
         */
        public boolean onRenameList(String list, String newList) { return true; }
    }
}
