...
public class Workspace {
    ...
    private final NameManager mListNameManager = new NameManager.ListNameManager();
    private final ConnectionManager mConnectionManager = new ConnectionManager();
    private final WorkspaceStats mStats =
            new WorkspaceStats(mVariableNameManager, mListNameManager, mProcedureManager,
                    mConnectionManager);
    ...
    public List<FieldList> getListRefs(String list) {
        List<FieldList> refs = mStats.getListReferences().get(list);
        List<FieldList> copy = new ArrayList<>(refs == null ? 0 : refs.size());
        if(refs != null) {
            copy.addAll(refs);
        }
        return copy;
    ...
    public int getListRefCount(String list) {
        List<FieldList> refs = mStats.getListReferences().get(list);
        return refs == null ? 0 : refs.size();
    }
    ...
    public List<Block> getBlocksWithList(String list, List<Block> resultList) {
        List<FieldList> refs = mStats.getListReferences().get(list);
        if(resultList == null) {
            resultList = new ArrayList<>();
        }
    ...
    public NameManager getListNameManager() { return mListNameManager; }
